...
namespace WebApp.Api.Admin
{
    ...
    {
        private string PageUrl = "/Admin/Restaurant";
        private string ApiName = "Restaurant";
        ...
        [Route("GetRestaurant")]
        public async Task<IHttpActionResult> GetRestaurant([FromUri] FilterModel param)
        {
            using (WebAppEntities db = new WebAppEntities())
            {
                try
                {
                    ...
                    IEnumerable<CustomRestaurant> source = null;
                    source = await (from ca in db.Restaurants
                                    select new CustomRestaurant
                                    {
                                        Id = ca.Id,
                                        Name = ca.Name,
                    ...
                    // sorting
                    var sortby = typeof(CustomRestaurant).GetProperty(param.sortby);
                    ...
                    var data = new { COUNT = source.Count(), RestaurantLIST = sourcePaged, CONTROLS = permissionCtrl };
                    return Ok(data);
                }
                catch (Exception ex)
                {
        ...
        [Route("UpdateStatus")]
        public async Task<IHttpActionResult> UpdateStatus(CustomRestaurant data)
        {
            ...
            using (WebAppEntities db = new WebAppEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var cd = db.Restaurants.Where(x => ids.Contains(x.Id)).Select(x => new { x.Id, x.Name, Published = x.Published.ToString() }).ToList();

                        foreach (var ds in data.dsList)
                        {
                            var sql = "Update Restaurant SET Published = {1}, ModifiedByPK = {2}, ModifiedDate = {3} WHERE Id = {0}";
        ...
        public async Task<IHttpActionResult> SaveRestaurant(Restaurant data)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            using (WebAppEntities db = new WebAppEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        bool isRestaurantnExists = db.Restaurants.Where(x => x.Name == data.Name && x.Id != data.Id).Any();
                        if (isRestaurantnExists)
        ...
        public IHttpActionResult RemoveData(int ID)
        {
            using (WebAppEntities db = new WebAppEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        var cd = db.Restaurants.Where(x => x.Id == ID).Select(x => new { x.Id, x.Name, Published = x.Published.ToString() }).SingleOrDefault();

                        db.Restaurants.RemoveRange(db.Restaurants.Where(x => x.Id == ID));
                        db.SaveChanges();
        ...
        [Route("RemoveRecords")]
        public async Task<IHttpActionResult> RemoveRecords(CustomRestaurant data)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            using (WebAppEntities db = new WebAppEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (var ds in data.dsList)
                        {
                            var sql = "DELETE FROM Restaurant WHERE Id = {0}";
