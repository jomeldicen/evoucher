app.controller("Property", ['$scope', '$http', '$window', '$timeout', '$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {

    var tokenKey = 'accessToken';
    document.body.style.cursor = 'default';
    $scope.data = { Id: 0, Name: '', Location: '', Email: '', ContactNos: '', Mobile: '', Published: 'True' };
    $scope.Property = [];
    ...
    $scope.GetPropertyData = function () {
        ...
        urlData = '../api/Property/GetProperty';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.Property = response.data.PropertyLIST;
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.Property, function (item) {
    ...
    $scope.AddData = function () {
        $scope.clearData();
        $scope.data = { Id: 0, Name: '', Published: 'True' };
    };
    // End Create control
    ...
    $scope.EditPropertyData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };
    ...
    $scope.SaveProperty = function (data) {
        ...
        if ($scope.PropertyForm.$invalid || $scope.PropertyForm.$invalid || data.PropertyImage === null) {
            swal(
                'Error Message',
            ...
            $scope.SavePropertyConfirmed(data);
        }
    }

    $scope.SavePropertyConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        document.body.style.cursor = 'wait';
        urlData = '../api/Property/SaveProperty';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        url = '../api/Property/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeleteProperty = function (data) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeletePropertyConfirmed(data);
            }
        });
    }

    $scope.DeletePropertyConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
