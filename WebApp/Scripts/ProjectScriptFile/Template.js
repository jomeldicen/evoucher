app.controller("Template", ['$scope', '$http', '$window', '$timeout', '$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {
    ...
    $scope.clearData = function () {
        ...
        document.body.style.cursor = 'default';
        $scope.data = { Id: 0, Name: '', Published: 'True' };
        $scope.data2 = { Id: '', TemplateID: 0, Amount: 0, Description: '', Condition: 'equal', TemplatePath: '' };
    ...
    $scope.GetTemplateData = function () {
        ...
        urlData = '../api/Template/GetTemplate';
        $http({
            method: 'GET',
            url: urlData,
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.Template, function (item) {
    ...
    $scope.EditTemplateData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };
    ...
    $scope.SaveTemplate = function (data) {
        ...
        if ($scope.TemplateForm.$invalid || $scope.TemplateForm.$invalid) {
            swal(
                'Error Message',
            ...
            $scope.SaveTemplateConfirmed(data);
        }
    }

    $scope.SaveTemplateConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        document.body.style.cursor = 'wait';
        urlData = '../api/Template/SaveTemplate';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        url = '../api/Template/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeleteTemplate = function (data) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeleteTemplateConfirmed(data);
            }
        });
    }

    $scope.DeleteTemplateConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
